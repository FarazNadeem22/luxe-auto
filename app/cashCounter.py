...
def menu(batch_number: int) -> int:
    print("*" * 50)
    print("Welcome to the Application!")
    print(f"Your current batch number is {batch_number}")
    print("1. Make a New Entry")
    print("2. Close Batch - This will automatically get you a new batch number")
    print("3. Get New Batch Number")
    print("4. Exit")
    print("*" * 50)
    while True:
...
def take_action(batch_number: int):
    while True:
        ...
        if choice == 3: # Get new Batch Numbr
            batch_number = get_batch_number(batch_number)
        elif choice == 1: # Make a new Entry
            new_entry = add_entry(batch_number=batch_number)
            print (new_entry)
            filename = str(batch_number)+".csv"
            append_entry_to_csv(new_entry=new_entry, csv_file=filename)
